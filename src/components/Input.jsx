...
const Input = forwardRef(function({label,textarea,...restprops},ref)
    ...
    return(
        <p className="flex flex-col gap-1 my-4">
            <label className="tesxt-sm text-stone-500 font-bold uppercase">{label}</label>
            {textarea ? <textarea ref={ref} className={classes} {...restprops}/> : <input ref={ref} className={classes} {...restprops}/>}
        </p>
    )
});
